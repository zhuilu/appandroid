...
public class PayPsdInputView extends AppCompatEditText {
     ...
     * 圆的颜色   默认BLACK
     */
    private int circleColor = getResources().getColor(R.color._969696);
    /**
     * 底部线的颜色   默认GRAY
     */
    private int bottomLineColor = getResources().getColor(R.color._969696);

    /**
     * 分割线的颜色
     */
    private int borderColor =  getResources().getColor(R.color._969696);
    /**
     * 分割线的画笔
     */
     ...
     * 竖直分割线的颜色
     */
    private int divideLineColor = getResources().getColor(R.color._969696);
    private int focusedColor = getResources().getColor(R.color._969696);
    ...
    private void drawText(Canvas canvas) {
        ...
        //    textPaint.setTextAlign(Paint.Align.CENTER);
            // 获取文本大小
//            canvas.drawText(text, startX + i * 2 * startX, canvas.getHeight() / 2, textPaint);

            //获取文字宽度
            float textWidth = textPaint.measureText(text, 0, text.length());
            float dx = startX + i * 2 * startX - textWidth / 2;
            Paint.FontMetricsInt fontMetricsInt = textPaint.getFontMetricsInt();
            float dy = (fontMetricsInt.bottom - fontMetricsInt.top) / 2 - fontMetricsInt.bottom;
            float baseLine = startY + dy;
        ...
        if (textLength == maxCount) {
            // 当前文本长度
                ...
                if (mOnInputFinishListener != null) {
                    mOnInputFinishListener.onTextFinish(getEditableText().toString(), maxCount);
