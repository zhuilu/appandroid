package com.xinniu.android.qiqueqiao.customs.calendarview.weiget;
...
import com.xinniu.android.qiqueqiao.customs.calendarview.listener.OnSingleChooseListener;
...
public class MonthView extends ViewGroup {
    ...
    public void setDateList(List<DateBean> dates, int currentMonthDays) {
        ...
        for (int i = 0; i < dates.size(); i++) {
            ...
            if (date.getMan().equals("满")) {
                solarDay.setTextColor(Color.parseColor("#DE6654"));
                lunarDay.setTextColor(Color.parseColor("#DE6654"));
                lunarDay.setText("已满");
...
//            }
            ...
            if (date.getType() == 1) {
                ...
                if (mAttrsBean.getDisableStartDate() != null
                    ...
                    solarDay.setTextColor(Color.parseColor("#ffcccccc"));
                    lunarDay.setTextColor(mAttrsBean.getColorLunar());
                    view.setTag(-1);
                    addView(view, i);
                    continue;
                        ...
                        if (mAttrsBean.getChooseType() == 1 && chooseListener != null) {//多选的情况
                            if (date.getMan().equals("满")) {
                                ...
                                if (chooseDays.contains(day)) {
                                    ...
                                    flag = false;
                                    Constants.mFixNum = Constants.mFixNum + 1;

                                } else {
                                    if (Constants.mFixNum == 0) {
                                        ToastUtils.showCentetToast(mContext, "置顶卡数量不足");
                                        return;
                                    }
                            ...
                            String time = date.getSolar()[0] + "-" + date.getSolar()[1] + "-" + date.getSolar()[2];
                            String time1 = mAttrsBean.getStartDate()[0] + "-" + mAttrsBean.getStartDate()[1] + "-" + mAttrsBean.getStartDate()[2];
                            Date data = CalendarUtil.parseServerTime(time, "");
                            Date data1 = CalendarUtil.parseServerTime(time1, "");
                            if (date.getMan().equals("满")) {
    ...
    private void setDayColor(View v, int type) {
        ...
        if (type == 0) {
            ...
            solarDay.setBackgroundResource(mAttrsBean.getDayBg());
