...
public  class IndexBar extends View {
    ...
    private List<SelectCityActivity.CityBean> mSourceDatas;//Adapter的数据源

    private List<GetFriendListBean.GroupBean> mSourceDatax;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mPaint.setColor(Color.parseColor("#2DA0FB"));
            ...
            public void onIndexPressed(int index, String text) {
                ...
                if (type == "city") {
                    if (mLayoutManager != null) {
                        int position = getPosByTag(text,type);
                        if (position != -1) {
    ...
    public IndexBar setmSourceDatas(List<SelectCityActivity.CityBean> mSourceDatas,String type) {
        this.mSourceDatas = mSourceDatas;
        this.type = type;
        initSourceDatas(type);//对数据源进行初始化
        return this;
    }
    public IndexBar setmSourceDatax(List<GetFriendListBean.GroupBean> mSourceDatas,String type) {
        this.mSourceDatax = mSourceDatas;
        this.type = type;
        initSourceDatas(type);//对数据源进行初始化
        return this;
    }
     ...
     * @return
     */
    private void initSourceDatas(String type) {
        //add by zhangxutong 2016 09 08 :解决源数据为空 或者size为0的情况,
...
//        }
        if (type == "city") {
            if (isNeedRealIndex) {
                for (int i = 0; i < mSourceDatas.size(); i++) {
                    mIndexDatas.add(mSourceDatas.get(i).tag);
                }
                computeGapHeight();
            }
        }else if (type == "friend"){
            if (isNeedRealIndex) {
                for (int i = 0; i < mSourceDatax.size(); i++) {
                    mIndexDatas.add(mSourceDatax.get(i).getName());
                }
                computeGapHeight();
            }
        }
     ...
     * @return
     */
    private int getPosByTag(String tag,String type) {
        //add by zhangxutong 2016 09 08 :解决源数据为空 或者size为0的情况,
        if (type == "city") {
            if (null == mSourceDatas || mSourceDatas.isEmpty()) {
                return -1;
            ...
            for (int i = 0; i < mSourceDatas.size(); i++) {
                if (tag.equals(mSourceDatas.get(i).tag)) {
                    return i + getHeaderViewCount();
                }
            }
        }else if (type == "friend") {
            if (null == mSourceDatax || mSourceDatax.isEmpty()) {
                return -1;
            }
            if (TextUtils.isEmpty(tag)) {
                return -1;
            ...
            for (int i = 0; i < mSourceDatax.size(); i++) {
                if (tag.equals(mSourceDatax.get(i).getName())) {
                    return i + getHeaderViewCount();
                }
            }
        }
