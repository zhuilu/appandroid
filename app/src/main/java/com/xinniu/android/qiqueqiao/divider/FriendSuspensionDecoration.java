...
public class FriendSuspensionDecoration extends RecyclerView.ItemDecoration {
    private List<GetFriendListBean.GroupBean> mDatas;
    ...
    private static int COLOR_TITLE_FONT = Color.parseColor("#FFF6F6F6");
    private static int COLOR_TITLE_TEXT = Color.parseColor("#FF999999");
    ...
    public FriendSuspensionDecoration(Context context, List<GetFriendListBean.GroupBean> datas) {
        super();
        mDatas = datas;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        paddingleft = DensityUtil.dp2px(15);
        mInflater = LayoutInflater.from(context);
    }


    public FriendSuspensionDecoration setmTitleHeight(int mTitleHeight) {
        this.mTitleHeight = mTitleHeight;
        return this;
    }

    public FriendSuspensionDecoration setmDatas(List<GetFriendListBean.GroupBean> mDatas) {
        this.mDatas = mDatas;
        return this;
    }
    ...
    public FriendSuspensionDecoration setHeaderViewCount(int headerViewCount) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0肯定要有title的
                    ...
                    if (null != mDatas.get(position).getName() && !mDatas.get(position).getName().equals(mDatas.get(position - 1).getName())) {
...
//        c.drawCircle(paddingleft+child.getPaddingLeft()+zhijing/2, child.getTop() - params.topMargin - mTitleHeight/2,zhijing ,mPaint);
        mPaint.setColor(COLOR_TITLE_TEXT);
        ...
        mPaint.getTextBounds(mDatas.get(position+1).getName(), 0, mDatas.get(position).getName().length(), mBounds);
        c.drawText(mDatas.get(position+1).getName(), paddingleft+child.getPaddingLeft(), baseLineY, mPaint);
    }
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        String tag = mDatas.get(pos+1).getName();
        ...
        if ((pos + 1) < mDatas.size()) {//防止数组越界（一般情况不会出现）
            if (null != tag && !tag.equals(mDatas.get(pos + 1).getName())) {//当前第一个可见的Item的tag，不等于其后一个item的tag，说明悬浮的View要切换了
...
//        float method1 = mPaint.measureText(tag);
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            GetFriendListBean.GroupBean titleCategoryInterface = mDatas.get(position);
//            //等于0肯定要有title的,
...
//            // 2016 11 10 add 通过接口里的isShowSuspension() 方法，先过滤掉不想显示悬停的item
            if (position == 0) {
                ...
                if (null != titleCategoryInterface.getName() && !titleCategoryInterface.getName().equals(mDatas.get(position - 1).getName())) {
                    //不为空 且跟前一个tag不一样了，说明是新的分类，也要title
                    outRect.set(0, mTitleHeight, 0, 0);
                }
            }
